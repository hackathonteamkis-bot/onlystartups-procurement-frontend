...
export const getPrograms = async () => {
  try {
    return await fetchWithAuth("/programs");
  } catch (error: any) {
...
export const createProgram = async (data: any) => {
  try {
    const res = await fetchWithAuth("/programs", {
      method: "POST",
      body: JSON.stringify(data),
    });
    revalidatePath("/startup-hub/programs");
...
export const updateProgram = async (id: string, data: any) => {
  try {
    const res = await fetchWithAuth(`/programs/${id}`, {
      method: "PATCH",
      body: JSON.stringify(data),
    });
    revalidatePath("/startup-hub/programs");
    revalidatePath(`/startup-hub/programs/${id}/customize`);
    return { success: true, data: res };
  } catch (error: any) {
...
export const getProgramById = async (id: string) => {
  try {
    return await fetchWithAuth(`/programs/${id}`, { cache: 'no-store' });
  } catch (error: any) {
