...
import { getStartupHubData, deleteFundingOpportunity } from "@/actions/startup-hub";
import { 
  Skeleton, 
  Badge,
  Button,
...
import { ShareSchemeDialog } from "@/components/startup-hub/share-scheme-dialog";
...
interface FundingOpportunity {
  ...
  startupHub?: { name: string; startupName?: string; image?: string };
  reports?: any[];
}
...
export default function FundingOpportunitiesPage() {
  ...
  const fetchData = useCallback(async () => {
    const res = await getStartupHubData();
    if (!("error" in res)) {
      setData(res);
    }
    setLoading(false);
    ...
    <div className="space-y-8 p-4 sm:p-6 pb-20">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-gradient-to-r from-[#1A1A2E] to-[#2a2a4a] p-8 rounded-xl text-white shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h1 className="text-3xl font-black mb-2 tracking-tight">Funding Opportunities</h1>
          <p className="text-white/70 max-w-xl text-sm leading-relaxed">
            Manage, track, and deploy capital for your startup ecosystem. Create tailored funding opportunities and track utilization across programs.
          </p>
        </div>
        <div className="shrink-0 relative z-10">
          <Link
            href="/startup-hub/funding-opportunities/new"
        ...
        {(!data?.fundingOpportunities || data.fundingOpportunities.length === 0) ? (
           <div className="py-16 flex flex-col items-center justify-center bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
             ...
             <p className="text-gray-500 font-medium">No funding opportunities created yet.</p>
           </div>
        ) : (
              ...
              <Table>
                <TableHeader>
                  <TableRow className="hover:bg-transparent border-gray-100">
                    <TableHead className="font-bold text-[10px] uppercase tracking-widest text-[#1A1A2E]/70 py-4 px-6">Opportunity Name</TableHead>
                        ...
                        <TableCell className="py-4 px-6 text-right">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-36 bg-white border border-gray-100 rounded-lg shadow-lg py-1">
                              ...
                              <DropdownMenuItem asChild className="cursor-pointer">
                                <Link href={`/startup-hub/funding-opportunities/new?id=${opportunity.id}`} className="w-full flex items-center gap-2">
