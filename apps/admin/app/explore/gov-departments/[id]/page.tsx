...
export default function GovDepartmentDetails() {
  const { id } = useParams();
  ...
  useEffect(() => {
    async function fetchGovDepartment() {
      try {
        setLoading(true);
        const data = await getGovDepartmentById(id as string);
        if (data) {
          setGovDepartment(data);
          ...
          if (currentUserId && currentUserId !== id && !hasTrackedRef.current) {
            ...
            void trackAnalytics("PROFILE_VIEW", "govDepartment_profile", id as string);
          }
        }
      } catch (error) {
