    ...
    const fetchProgram = async () => {
      try {
        ...
        if (!res || "error" in res) {
          ...
          setBreadcrumb(params.id, res.startupHub?.startupName || res.startupHub?.name || params.id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.error) {
        ...
        setTimeout(() => {
          router.push(`/explore/startup-hubs/${params.id}/programs/${params.programId}`);
        }, 2500);
      }
    } catch (err) {
    ...
    <div className="space-y-6 pb-20 min-h-[80dvh] flex flex-col justify-between w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="space-y-4 pt-6">
        <div className="flex items-center justify-between text-[10px] sm:text-xs font-black text-muted-foreground uppercase tracking-widest">
          <Link
            href={`/explore/startup-hubs/${params.id}/programs/${params.programId}`}
