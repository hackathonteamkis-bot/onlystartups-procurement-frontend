...
export function StartupHubDashboard({ initialData }: { initialData?: StartupHubStats }) {
  const { data: session } = useSession();
  const [currentTime, setCurrentTime] = useState(new Date());
  const [statsData, setStatsData] = useState<StartupHubStats | null>(initialData || null);
  const [loading, setLoading] = useState(!initialData);

  const fetchData = useCallback(async () => {
    const data = await getStartupHubStats();
    if (!("error" in data)) {
      setStatsData(data);
    }
    setLoading(false);
  ...
  useEffect(() => {
    if (!session?.user?.id || session.user.role !== "STARTUP_HUB") return;

    const userId = session.user.id;

    // Subscribe to applications
    const appsChannel = supabase
      .channel(`startup-hub-apps-${userId}`)
      .on(
        "postgres_changes",
        {
          event: "INSERT",
          schema: "public",
          table: "applications",
          filter: `startupHub_id=eq.${userId}`,
        },
        (_payload) => {
    ...
    const milestonesChannel = supabase
      .channel(`startupHub-milestones-${userId}`)
      .on(
        "postgres_changes",
        {
          event: "*",
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="border-none shadow-xl bg-white/80 backdrop-blur-sm flex flex-col">
          ...
          <div className="p-4 mt-auto">
            <Link href="/startup-hub/applications" className="w-full py-2 bg-[#1A1A2E] text-white rounded-xl text-center text-xs font-bold flex items-center justify-center gap-2 hover:bg-[#F26522] transition-colors">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-1">
          <SharingCard
            ...
            startupHubName={session?.user?.startupName || `${session?.user?.firstName || ''} ${session?.user?.lastName || ''}`.trim() || "OnlyStartups"}
          />
        </div>
        <div className="lg:col-span-2">
          <AnalyticsSummary startupHubId={session?.user?.id || ""} />
        </div>
      </div>
