...
import { getEventById, updateEventRegistrationStatus, deleteEvent } from "@/actions/gov-department/events";
  ...
  const handleDeleteEvent = async () => {
    ...
    try {
      ...
      if (res.error) {
        ...
        router.push("/gov-department/events");
      }
    } catch (e) {
  ...
  if (error || !event) {
    return (
      <div className="p-12 text-center bg-white rounded-xl border border-red-100">
        ...
        <Link href="/gov-department/events">
  ...
  return (
    <div className="space-y-6 pb-20">
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div>
          <Link href="/gov-department/events" className="text-xs font-black text-muted-foreground flex items-center gap-2 hover:text-[#1A1A2E] transition-colors tracking-widest uppercase mb-4 w-fit">
        ...
        <div className="flex items-center gap-2">
          <Link href={`/gov-department/events/new?id=${eventId}`}>
