...
import { createEvent, updateEvent, getEventById } from "@/actions/gov-department/events";
...
function EventWizardContent() {
    ...
    try {
      ...
      if (editId) {
        ...
        if (res.error) {
          ...
          router.push(`/gov-department/events/${editId}`);
        }
      } else {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto space-y-8 pb-20 p-4 sm:p-6">
      <Link href="/gov-department/events" className="text-xs font-black text-muted-foreground flex items-center gap-2 hover:text-[#1A1A2E] transition-colors tracking-widest uppercase w-fit">
