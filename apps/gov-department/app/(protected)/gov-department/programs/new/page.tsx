...
import { getProgramById, createProgram, updateProgram } from "@/actions/gov-department/programs";
import { getSignedUploadUrl } from "@/actions/user/storage";
import { supabase } from "@/lib/supabase";
import { ProgramFormBuilder, Question } from "@/components/gov-department/program-form-builder";
...
function WizardContent() {
    ...
    try {
      ...
      if (programId) {
        ...
        if (res.error) {
          ...
          router.replace(`/gov-department/programs/new?id=${res.data.id}`);
          setStep(2);
        }
      }
    } catch (e) {
  ...
  const handleLaunch = async () => {
    ...
    try {
      ...
      router.push(`/gov-department/programs/${programId}`);
    } catch (e) {
    ...
    <div className="max-w-5xl mx-auto space-y-8 pb-20 pt-6 px-4 md:px-0 transition-all duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        <div className="space-y-1">
          <Link href="/gov-department/programs" className="text-xs font-bold text-muted-foreground hover:text-primary transition-colors flex items-center gap-2 mb-4">
            <ArrowLeft className="w-3.5 h-3.5" /> BACK TO PROBLEM STATEMENTS
          </Link>
          <h1 className="text-3xl font-black text-[#1A1A2E] tracking-tight">Deploy Problem Statement</h1>
