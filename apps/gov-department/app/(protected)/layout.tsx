...
export default async function ProtectedLayout({
  ...
  // Redirect to setup-password if user needs to change their temporary password
  if (session?.user?.needsPasswordChange) {
    redirect("/auth/setup-password");
  }

  // Redirect to onboarding if user hasn't completed it (Admins bypass this)
  if (session?.user && !session.user.onboardingComplete && session.user.role !== "ADMIN") {
    redirect("/onboarding");
  }
