...
import { updateFundingOpportunity } from "@/actions/gov-department";
...
export function EditSchemeDialog({ scheme, onSuccess }: EditSchemeDialogProps) {
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState({
    name: scheme.name,
    fundAmount: scheme.fundAmount || "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await updateFundingOpportunity(scheme.id, formData);
      if (res.error) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <button className="flex items-center gap-2 px-6 py-2.5 h-10 rounded-full bg-[#1A1A2E] text-white font-bold hover:bg-[#F26522] transition-all text-sm">
          Edit <Pencil className="w-3.5 h-3.5 ml-1" />
        </button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[600px] rounded-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="text-sm font-medium">
            Update the details for &quot;{scheme.name}&quot;. Changes will be live
            immediately.
          </DialogDescription>
        </DialogHeader>
