...
export default function StartupHubDetailPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const found = await getStartupHubById(params.id as string);
        ...
        if (found && currentUserId && currentUserId !== params.id && !hasTrackedRef.current) {
          ...
          void trackAnalytics("PROFILE_VIEW", "startupHub_profile", params.id as string);
        }
      } catch (error) {
