...
interface SharingCardProps {
    ...
    type?: "startupHub_form" | "funding_opportunity";
    itemKey?: string;
    customUrl?: string;
    title?: string;
    description?: string;
...
export function SharingCard({
    ...
    const shareUrl = customUrl || `${baseUrl}/join/${startupHubId}`;
    ...
    const copyToClipboard = async () => {
        try {
            ...
            await trackAnalytics("LINK_COPY", analyticsKey, startupHubId);
    ...
    const downloadQr = () => {
        ...
        img.onload = () => {
            ...
            downloadLink.download = `${startupHubName.replace(/\s+/g, "-")}-qr.png`;
            downloadLink.href = pngFile;
            downloadLink.click();

            // Track QR download
            trackAnalytics("QR_DOWNLOAD", "startupHub_form", startupHubId);
        };
                ...
                <Dialog open={showQr} onOpenChange={setShowQr}>
                    <DialogContent className="sm:max-w-md rounded-xl p-8 flex flex-col items-center">
                        ...
                        <div className="mt-8 text-center space-y-1">
                            ...
                            <p className="text-lg font-bold text-[#F26522]">{startupHubName}</p>
                        </div>
