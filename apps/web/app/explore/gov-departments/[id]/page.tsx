...
import { getGovDepartmentById } from "@/actions/explore";
...
interface GovDepartment {
  ...
  email?: string;
  networkSize?: string;
  govDepartmentQuote?: string;
  isApplied?: boolean;
  location?: string;
  sectors?: string[];
  programDuration?: string;
  equityTaken?: string;
  govDepartmentSocials?: Record<string, string>;
...
export default function GovDepartmentDetailPage() {
  const params = useParams();
  const router = useRouter();
  const { data: session, status } = useSession();
  const { setBreadcrumb } = useBreadcrumbContext();
  const [govDepartment, setGovDepartment] = useState<GovDepartment | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const found = await getGovDepartmentById(params.id as string);
        const currentUserId = session?.user?.id;

        if (found) {
          setGovDepartment(found);
          setBreadcrumb(params.id as string, found.startupName || found.name);
          
          const publicPrograms = await getPublicPrograms(params.id as string);
          if (Array.isArray(publicPrograms) && publicPrograms.length > 0) {
            setPrograms(publicPrograms);
          }
        }

        // Track the view if found AND it's not a self-visit AND we haven't tracked yet
        if (found && currentUserId && currentUserId !== params.id && !hasTrackedRef.current) {
          hasTrackedRef.current = true;
          void trackAnalytics("PROFILE_VIEW", "govDepartment_profile", params.id as string);
        }
      } catch (error) {
  ...
  }, [params.id, session?.user?.id, setBreadcrumb]);
  ...
  if (!govDepartment) {
    return (
      <div className="h-[80vh] flex flex-col items-center justify-center gap-4 text-center">
        ...
        <h2 className="text-xl font-bold">Gov Department not found</h2>
        <Link
  ...
  const stats = [
    {
      ...
      value: govDepartment.activeStartups || " ",
      icon: Rocket,
    },
    {
      label: "Total Exits",
      value: govDepartment.totalExits || " ",
      icon: ArrowUpRight,
    },
    {
      label: "Funding Raised",
      value: govDepartment.fundingRaised ? `INR ${govDepartment.fundingRaised}` : " ",
      icon: Wallet,
    },
    { label: "Mentors", value: govDepartment.mentorCount || " ", icon: Users2 },
  ];

  const hasAnyStats =
    govDepartment.activeStartups ||
    govDepartment.totalExits ||
    govDepartment.fundingRaised ||
    govDepartment.mentorCount;
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div>
        ...
        <div className="relative rounded-2xl overflow-hidden border border-[#1A1A2E]/5 h-36 sm:h-52 w-full bg-gradient-to-r from-[#1A1A2E] via-[#2A2A44] to-[#F26522]/30 shadow-sm">
          ...
          {govDepartment.bannerImage && (
            <Image
              src={govDepartment.bannerImage}
              alt="Cover Banner"
              fill
        ...
        <div className="px-4 sm:px-6 relative z-10 text-left">
          <div className="flex flex-row items-start gap-4 sm:gap-6">
            <Avatar className="h-20 w-20 sm:h-32 sm:w-32 lg:h-36 lg:w-36 rounded-full border-4 border-[#F5F5EE] bg-white shadow-md shrink-0 -mt-4 sm:-mt-8">
              {govDepartment.image && <AvatarImage src={govDepartment.image} className="object-cover animate-in fade-in" />}
              <AvatarFallback className="text-2xl sm:text-4xl font-black bg-[#1A1A2E] text-white">
                {govDepartment.startupName?.charAt(0) || govDepartment.name?.charAt(0)}
              </AvatarFallback>
            </Avatar>
                ...
                <div className="space-y-1.5 sm:space-y-2">
                  <div className="flex items-center justify-start gap-1 sm:gap-2 flex-wrap">
                    <h1 className="text-xl sm:text-3xl font-black text-[#1A1A2E] tracking-tight leading-tight sm:leading-none">
                      {govDepartment.startupName || govDepartment.name}
                    </h1>
                  </div>
                  {govDepartment.location && (
      ...
      <div className="flex flex-col gap-8 pb-20">
        ...
        {programs.length > 0 && (
          <section className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex items-center justify-between">
              ...
              {programs.length > 3 && (
                <Link
                  href={`/explore/gov-departments/${govDepartment.id}/programs`}
                ...
                <div key={program.id} className="bg-white border border-muted/50 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:border-primary/20 transition-all group">
                  ...
                  <div className="p-4 space-y-3">
                    ...
                    <div className="flex items-center gap-2">
                      <Link
                        href={program.applyUrl || `/explore/gov-departments/${govDepartment.id}/programs/${program.id}/apply`}
                        onClick={(e) => {
                      ...
                      <Link
                        href={`/explore/gov-departments/${govDepartment.id}/programs/${program.id}`}
        ...
        {/* Overview & Tagline */}
        {(govDepartment.startupDescription || govDepartment.bio || govDepartment.govDepartmentQuote) && (
          <section className="bg-white border border-muted/50 p-6 sm:p-8 rounded-2xl space-y-8 shadow-sm">
            
            {govDepartment.govDepartmentQuote && (
            ...
            {(govDepartment.startupDescription || govDepartment.bio) && (
              <div className="space-y-3">
                <h3 className="text-base font-black text-[#1A1A2E] flex items-center gap-2 uppercase tracking-wider">
                  <span className="w-1 h-4 bg-[#F26522] rounded-full" />
                  Overview
        ...
        {!govDepartment.startupDescription && !govDepartment.bio && !govDepartment.govDepartmentQuote && (
          <div className="py-16 flex flex-col items-center justify-center text-center space-y-3 bg-muted/5 rounded-2xl border-2 border-dashed border-muted/30">
            ...
            <p className="text-sm text-muted-foreground font-semibold">
              This government department hasn&apos;t added an overview yet.
            </p>
          </div>
        )}
        ...
        <div className="bg-gradient-to-br from-[#1A1A2E] to-[#2B2B3E] text-white p-6 sm:p-8 rounded-2xl space-y-6 border border-white/5 shadow-xl relative overflow-hidden">
            ...
            {govDepartment.location && (
              <div className="flex items-center gap-4 group">
                ...
                <div>
                  ...
                  <p className="text-xs sm:text-sm font-semibold text-white/95">{govDepartment.location}</p>
                </div>
              </div>
            )}
            {govDepartment.programDuration && (
              <div className="flex items-center gap-4 group">
                <div className="w-9 h-9 rounded-xl bg-white/5 flex items-center justify-center text-[#F26522] group-hover:bg-[#F26522]/10 transition-colors shrink-0">
                ...
                <div>
                  ...
                  <p className="text-xs sm:text-sm font-semibold text-white/95">{govDepartment.programDuration}</p>
                </div>
              </div>
            )}
            {govDepartment.equityTaken && (
              <div className="flex items-center gap-4 group">
                <div className="w-9 h-9 rounded-xl bg-white/5 flex items-center justify-center text-[#F26522] group-hover:bg-[#F26522]/10 transition-colors shrink-0">
                ...
                <div>
                  ...
                  <p className="text-xs sm:text-sm font-semibold text-white/95">{govDepartment.equityTaken}</p>
                </div>
              </div>
            )}
            {govDepartment.networkSize && (
              <div className="flex items-center gap-4 group">
                <div className="w-9 h-9 rounded-xl bg-white/5 flex items-center justify-center text-[#F26522] group-hover:bg-[#F26522]/10 transition-colors shrink-0">
                ...
                <div>
                  ...
                  <p className="text-xs sm:text-sm font-semibold text-white/95">
                    {govDepartment.networkSize} Members
                  </p>
                </div>
              </div>
            )}
          ...
          {govDepartment.govDepartmentSocials && Object.entries(govDepartment.govDepartmentSocials).filter(([, url]) => hasValue(url)).length > 0 && (
            <div className="pt-6 border-t border-white/10 space-y-3">
              <p className="text-[9px] font-black text-white/40 uppercase tracking-widest">
                Social Channels
              </p>
              <div className="flex flex-wrap gap-2">
                {Object.entries(govDepartment.govDepartmentSocials)
