...
export default function GovDepartmentsPage() {
  const [data, setData] = useState<GovDepartment[]>([]);
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        setData((res as { govDepartments?: GovDepartment[] }).govDepartments || []);
      } catch (error) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="space-y-4">
        ...
        <PageHeader
          title="Gov Departments"
          description="Find top government departments to partner with for your startup."
        >
          <div className="flex items-center gap-2">
            ...
            <Dialog>
              ...
              <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                  <DialogTitle>Filter Gov Departments</DialogTitle>
                </DialogHeader>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <AnimatePresence mode="wait">
          {paginatedData.map((item: GovDepartment) => (
      ...
      {filteredData.length === 0 && (
        <div className="py-20 flex flex-col items-center justify-center text-center space-y-4 bg-muted/10 rounded-2xl border-2 border-dashed border-muted">
          ...
          <p className="text-sm font-medium text-muted-foreground">
            No government departments found matching your criteria.
          </p>
        </div>
      )}
    </div>
...
function CardMinimal({ item }: { item: GovDepartment }) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-4 flex items-center gap-2">
        ...
        <Link
          href={`/explore/gov-departments/${item.id}`}
