...
export default async function ProgramDetailPage({
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 px-4 sm:px-0 pb-20">
      <BreadcrumbUpdater id={id} name={program.startupHub?.startupName || program.startupHub?.name || "Startup Hub"} />
             ...
             <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6">
                ...
                <div className="shrink-0 w-full sm:w-auto">
                  {program.isApplied ? (
                    <Link
                      key="btn-submitted"
                      href={`/explore/startup-hubs/${id}/programs/${programId}/apply`}
                    ...
                    <Link
                      key="link-apply"
                      href={isLoggedIn 
                        ? (program.applyUrl || `/explore/startup-hubs/${id}/programs/${programId}/apply`) 
                        : `/auth/login?callbackUrl=${encodeURIComponent(program.applyUrl ? `/explore/startup-hubs/${id}/programs/${programId}` : `/explore/startup-hubs/${id}/programs/${programId}/apply`)}`}
                    ...
                    >
                      Apply Now {program.applyUrl && <ArrowUpRight className="w-4 h-4" />}
                    </Link>
                  ) : (
